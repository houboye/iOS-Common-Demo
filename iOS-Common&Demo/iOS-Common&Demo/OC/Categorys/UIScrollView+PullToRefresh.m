...
static CGFloat const BYPullToRefreshViewHeight = 60;

@interface BYPullToRefreshArrow : UIView
...
@interface BYPullToRefreshView ()

@property (nonatomic, copy) void (^pullToRefreshActionHandler)(void);

@property (nonatomic, strong) BYPullToRefreshArrow *arrow;
@property (nonatomic, strong) UIActivityIndicatorView *activityIndicatorView;
@property (nonatomic, strong, readwrite) UILabel *titleLabel;
@property (nonatomic, strong, readwrite) UILabel *subtitleLabel;
@property (nonatomic, readwrite) BYPullToRefreshState state;
@property (nonatomic, readwrite) BYPullToRefreshPosition position;
...
#pragma mark - UIScrollView (BYPullToRefresh)
#import <objc/runtime.h>

static char UIScrollViewPullToRefreshView;

@implementation UIScrollView (BYPullToRefresh)
...
- (void)addPullToRefreshWithActionHandler:(void (^)(void))actionHandler position:(BYPullToRefreshPosition)position {
    ...
    if(!self.pullToRefreshView) {
        CGFloat yOrigin;
        switch (position) {
            case BYPullToRefreshPositionTop:
                yOrigin = -BYPullToRefreshViewHeight;
                break;
            case BYPullToRefreshPositionBottom:
                yOrigin = self.contentSize.height;
                break;
            default:
                return;
        }
        BYPullToRefreshView *view = [[BYPullToRefreshView alloc] initWithFrame:CGRectMake(0, yOrigin, self.bounds.size.width, BYPullToRefreshViewHeight)];
...
- (void)addPullToRefreshWithActionHandler:(void (^)(void))actionHandler {
    [self addPullToRefreshWithActionHandler:actionHandler position:BYPullToRefreshPositionTop];
}

- (void)triggerPullToRefresh {
    self.pullToRefreshView.state = BYPullToRefreshStateTriggered;
    [self.pullToRefreshView startAnimating];
}

- (void)setPullToRefreshView:(BYPullToRefreshView *)pullToRefreshView {
...
- (BYPullToRefreshView *)pullToRefreshView {
...
- (void)setShowsPullToRefresh:(BOOL)showsPullToRefresh {
    ...
    if(!showsPullToRefresh) {
        ...
        if (!self.pullToRefreshView.isObserving) {
            ...
            self.pullToRefreshView.frame = CGRectMake(0, yOrigin, self.bounds.size.width, BYPullToRefreshViewHeight);
        }
    }
}
...
#pragma mark - BYPullToRefresh
@implementation BYPullToRefreshView
...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        ...
        self.state = BYPullToRefreshStateStopped;
...
- (void)layoutSubviews {
    ...
    if(hasCustomView) {
        ...
        switch (self.state) {
            case BYPullToRefreshStateAll:
            case BYPullToRefreshStateStopped:
                self.arrow.alpha = 1;
            ...
            case BYPullToRefreshStateTriggered:
                switch (self.position) {
                    case BYPullToRefreshPositionTop:
                        [self rotateArrow:(float)M_PI hide:NO];
                        break;
            ...
            case BYPullToRefreshStateLoading:
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if([keyPath isEqualToString:@"contentOffset"])
        ...
        self.frame = CGRectMake(0, yOrigin, self.bounds.size.width, BYPullToRefreshViewHeight);
    }
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset {
    if(self.state != BYPullToRefreshStateLoading) {
        ...
        if(!self.scrollView.isDragging && self.state == BYPullToRefreshStateTriggered)
            self.state = BYPullToRefreshStateLoading;
        else if(contentOffset.y < scrollOffsetThreshold && self.scrollView.isDragging && self.state == BYPullToRefreshStateStopped && self.position == BYPullToRefreshPositionTop)
            self.state = BYPullToRefreshStateTriggered;
        else if(contentOffset.y >= scrollOffsetThreshold && self.state != BYPullToRefreshStateStopped && self.position == BYPullToRefreshPositionTop)
            self.state = BYPullToRefreshStateStopped;
        else if(contentOffset.y > scrollOffsetThreshold && self.scrollView.isDragging && self.state == BYPullToRefreshStateStopped && self.position == BYPullToRefreshPositionBottom)
            self.state = BYPullToRefreshStateTriggered;
        else if(contentOffset.y <= scrollOffsetThreshold && self.state != BYPullToRefreshStateStopped && self.position == BYPullToRefreshPositionBottom)
            self.state = BYPullToRefreshStateStopped;
    } else {
        CGFloat offset;
...
- (BYPullToRefreshArrow *)arrow {
    if(!_arrow) {
        _arrow = [[BYPullToRefreshArrow alloc]initWithFrame:CGRectMake(0, self.bounds.size.height-54, 22, 48)];
...
- (void)setTitle:(NSString *)title forState:(BYPullToRefreshState)state {
    ...
    if(state == BYPullToRefreshStateAll)
...
- (void)setState:(BYPullToRefreshState)newState {
    
    if(_state == newState)
        return;
    
    BYPullToRefreshState previousState = _state;
    _state = newState;
    ...
    switch (newState) {
        ...
        case BYPullToRefreshStateLoading:
            ...
            if(previousState == BYPullToRefreshStateTriggered && pullToRefreshActionHandler)
...
#pragma mark - BYPullToRefreshArrow

@implementation BYPullToRefreshArrow
